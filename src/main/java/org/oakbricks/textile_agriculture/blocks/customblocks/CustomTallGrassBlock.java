...
public class CustomTallGrassBlock extends TallPlantBlock implements Fertilizable {
    ...
    public CustomTallGrassBlock(int delay, Settings s) {
        super(s);
        growthDelay = delay;
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (state.get(AGE).equals(7)) {
            ...
            ItemScatterer.spawn(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(TAItems.WHOLE_AVOCADO, count));
            if (state.get(HALF).equals(DoubleBlockHalf.UPPER)) {
    ...
    public void applyGrowth(World world, BlockPos pos, BlockState state) {
        int age = state.get(AGE);
        int newAge = CustomTallGrassBlock.getAge(state) + CustomTallGrassBlock.getGrowthAmount(world);
        if (newAge > 7) {
            newAge = 7;
        }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(TAItems.AVOCADO_PIT);
    }

    @Override
    public boolean isFertilizable(BlockView world, BlockPos pos, BlockState state, boolean isClient) {
        return !CustomTallGrassBlock.isMature(state);
    }
